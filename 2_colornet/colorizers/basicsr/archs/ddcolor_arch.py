...
class Encoder(nn.Module):
    ...
    def __init__(self, encoder_name, hook_names, **kwargs):
        ...
        if encoder_name == 'convnext-t' or encoder_name == 'convnext':
            self.arch = ConvNeXt()
            self.load('DDColor-master/ppretrain/convnext_tiny_22k_224.pth')
        elif encoder_name == 'convnext-s':
            self.arch = ConvNeXt(depths=[3, 3, 27, 3], dims=[96, 192, 384, 768])
            self.load('DDColor-master/ppretrain/convnext_small_22k_224.pth')
            ...
            self.load('D:\\anconda\\shibie\\DDColor-master\\pretrain\\convnext_large_22k_224.pth')
        else:
